...
-- 2-2-2. 투표 항목 삭제 (soft delete)
UPDATE tbl_vote_option SET 
is_deleted = 'Y'
WHERE vote_id = 1 
  AND is_deleted = 'N';
  
-- 2-2-3. 투표 삭제 (soft delete)
UPDATE tbl_vote SET 
is_deleted = 'Y'
WHERE vote_id = 1 
  AND is_deleted = 'N';
...
-- 2-3-1. 투표 history 삭제 (hard delete)
DELETE FROM tbl_vote_history
WHERE option_id = 1;  

-- 2-3-2. 투표 항목 삭제 (soft delete)
UPDATE tbl_vote_option SET 
is_deleted = 'Y'
WHERE option_id = 1 
  AND is_deleted = 'N';  
